...
const authErrorCodes: Record<string, string> = {
  ...
  weak_password: "Password must be at least 8 characters long and contain at least one number",
...
export const displayErrorCode = (errorCode: string): string => {
    const key = errorCode
    return authErrorCodes[key] || "An unexpected error occurred. Please try again";
  };