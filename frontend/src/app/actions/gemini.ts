...
export async function cloudCheckGeminiKey(userId: string) {
    ...
    if (!response.success) {
        return { success: false, error: response.error, exist: false, lastUpdated: null }
    }
    ...
    return { success: true, error: null, exist: true, lastUpdated: response.data?.updated_at }
...
export async function cloudPromptGemini(userId: string, prompt: string, context?: string, chatHistory?: string[], questionImage?: any) {
