            ...
            {aiProviders.map((provider) => (
                <div className="flex_center gap-8">
                    <div className="w-[60px] flex_col_center gap-1">
                        <Image alt={provider.title} src={provider.icon} height={20} width={20}/>
                        <span className="text-xs text-center text-muted-foreground">{provider.title}</span>
                    </div>
                    {provider.cmgSoon ?
                    <div className="w-full ">
                        <span className="ml-3 px-2 py-1 font-bold text-xs rounded-full bg-amber-600 text-gray-100">Coming Soon!</span>
                    </div>
                    : <SettingForm
                        key={provider.prefix}
